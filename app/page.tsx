  ...
  return (
    <div style={{ textAlign: 'right' }}>
      <div style={{
        ...
        textShadow: 'var(--glow-cyan)', lineHeight: 1,
      }}>
        {time}
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-void)', display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{
        ...
        position: 'sticky', top: 0, background: 'var(--bg-void)', zIndex: 100,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{
              ...
              color: 'var(--accent-cyan)', letterSpacing: '0.2em', textShadow: 'var(--glow-cyan)',
            }}>
          ...
          <div style={{ display: 'flex', gap: 20 }}>
            {[
              ...
              <div key={s.label} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 18, fontFamily: 'var(--font-display)', color: 'var(--accent-cyan)', fontWeight: 700 }}>
                  {s.value}
                </div>
              ...
              <div style={{ marginTop: 20 }}>
                ...
                {p.milestones.map((m) => (
                  <div key={m.id} style={{
                    ...
                    marginBottom: 12, opacity: m.completed ? 0.4 : 1,
                  }}>
                    <div style={{
                      width: 8, height: 8, borderRadius: '50%',
                      flexShrink: 0, marginTop: 2,
                      background: m.completed ? p.color : 'transparent',
                      border: `1px solid ${p.color}`,
                      boxShadow: m.completed ? `0 0 6px ${p.color}` : 'none',
                    }} />
                    <div>
                      <div style={{ fontSize: 10, color: m.completed ? 'var(--text-muted)' : 'var(--text-bright)', lineHeight: 1.4 }}>
                        {m.completed && <span style={{ marginRight: 4 }}>✓</span>}
                        {m.label}
                      </div>
                      <div style={{ fontSize: 8, color: 'var(--text-muted)', marginTop: 2 }}>
                        {new Date(m.targetDate).toLocaleDateString('ja-JP')}
                      </div>
                    </div>
                  </div>
                ))}
